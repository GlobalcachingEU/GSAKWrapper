            ...
            if (el != null)
            {
                ...
                if (ri.tag == "area" && ri.Brush == null && !string.IsNullOrEmpty(ri.src))
                {
                    try
                    {
                        ...
                        else
                        {
                            ...
                            using (Bitmap img = (Bitmap)Image.FromStream(Assembly.GetExecutingAssembly().GetManifestResourceStream(string.Format("GSAKWrapper.MapProviders.OSMBinMap.patterns.{0}", fn))))
                            {
                                ri.Brush = new TextureBrush(img);
                                (ri.Brush as TextureBrush).WrapMode = System.Drawing.Drawing2D.WrapMode.Tile;
                            }
                ...
                if (ri.tag == "symbol" && !string.IsNullOrEmpty(ri.src))
                {
                    try
                    {
                        ...
                        {
                            fn = Path.GetFileName(ri.src);
                            ri.Symbol = Image.FromStream(Assembly.GetExecutingAssembly().GetManifestResourceStream(string.Format("GSAKWrapper.MapProviders.OSMBinMap.symbols.{0}", fn)));
                        }
                    }
                    catch
                    {
